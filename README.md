...
This repo contains files completely from Phira, but they are not used inside the game. (e.g. staff.yml)


此 repo 帶有完全來源於Phira專案的非程式碼檔案，但它們並未在XC-SIM中使用。（例 staff.yml）


Original Phira assets are not compatible with XC-SIM and will fail to launch if you use it.


原版Phira的assets並不適用於XC-SIM，如果您堅持使用，將會無法啓動。
## Resource Packs 資源包
...
如果你正在使用一個帶有自定義hitFx/判定線顏色的資源包，請先確保顏色code為此格式：0xe1xxxxxx，或它可能不會正確顯示。（例 缺失的hitFx動效）
## Download 下載連結
...
Other language files inside this repository are considered as **INCOMPLETE** translation, and is not avaliable in the current release.


目前，只有英文和繁體中文的翻譯是可用的。


其餘語言檔案被視為**未完善**的翻譯，且不會在目前版本中可用。
## Links 連結
