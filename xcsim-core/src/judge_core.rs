...
static ARC_SCORE_ENABLED: AtomicBool = AtomicBool::new(false);

pub const FLICK_SPEED_THRESHOLD: f32 = 0.180;
pub const UP_TOLERANCE: f32 = 0.05;
pub const DIST_FACTOR: f32 = 0.2;
const EARLY_OFFSET: f32 = 0.00;
pub const LIMIT_BAD: f32 = 0.200;
#[derive(Debug, Clone)]
pub enum HitSound {
    None,
    Click,
...
#[cfg(not(feature = "closed"))]
impl JudgeInner {
        pub fn set_arcscore(&mut self, v: bool) {
        self.arcscore = v;
    }
    ...
    pub fn accuracy(&self) -> f64 {
        (self.counts[0] as f64 + self.counts[1] as f64 + self.counts[2] as f64 * 0.090 ) / self.num_of_notes as f64
    }
    ...
    pub fn real_time_accuracy(&self) -> f64 {
        ...
        (self.counts[0] as f64 + self.counts[1] as f64 + self.counts[2] as f64 * 0.090) / cnt as f64
    }
    ...
    const ARC_TOTAL_S: u32 = 10000000;
    let ARC_TOTAL: u32 = 10000000 + self.num_of_notes;
    if self.arcscore {
        let base = ARC_TOTAL_S as f64 / self.num_of_notes as f64;
        let base_arc = ARC_TOTAL as f64 / self.num_of_notes as f64;
        score =
           self.counts[0] as f64 * base +
           (self.counts[1] as f64 * base_arc ) + (self.counts[2] as f64 * base * 0.090);
    } else {
        let base = TOTAL as f64 / self.num_of_notes as f64;
        score =
            self.counts[0] as f64 * base +
            self.counts[1] as f64 * base + self.counts[2] as f64 * base * 0.400
            - (self.counts[2] + self.counts[3]) as f64 * (TOTAL as f64 * 0.000080);
...
impl Judge {
    ...
    pub fn update(&mut self, res: &mut Resource, chart: &mut Chart, bad_notes: &mut Vec<BadNote>) {
        self.inner.set_arcscore(res.config.arcscore);
        ARC_SCORE_ENABLED.store(res.config.arcscore, Ordering::Relaxed);

        if res.config.strict {
            self.limit_arc_perfect = 0.025;
            self.limit_perfect = 0.050;
            self.limit_good = 0.100;
            self.limit_bad = 0.115;
        } else {
            self.limit_arc_perfect = 0.050;
            self.limit_perfect = 0.080;
            self.limit_good = 0.160;
            self.limit_bad = 0.200;
        }
                    ...
                    if dt <= self.limit_good || matches!(note.kind, NoteKind::Hold { .. }) {
                        match note.kind {
                            NoteKind::Click => {
                                ...
                                judgements.push((if dt <= self.limit_arc_perfect { Judgement::ArcPerfect } else if dt <= self.limit_perfect { Judgement::Perfect } else { Judgement::Good }, line_id, id, Some(t)));
                            }
                            NoteKind::Hold { .. } => {
                                note.hitsound.play(res);
                                self.judgements.borrow_mut().push((t, line_id as _, id, Err(dt <= self.limit_perfect)));
                                note.judge = JudgeStatus::Hold(dt <= self.limit_perfect, t, t, false, f32::INFINITY);
                            }
                            _ => unreachable!(),
                        };
                    } else {
                ...
                if dt <= if matches!(note.kind, NoteKind::Click) { self.limit_bad } else { self.limit_good } {
                    match note.kind {
                        ...
                        NoteKind::Hold { .. } => {
                            note.hitsound.play(res);
                            self.judgements.borrow_mut().push((t, line_id as _, id, Err(dt <= self.limit_perfect)));
                            note.judge = JudgeStatus::Hold(dt <= self.limit_perfect, t, (t - note.time) / spd, false, f32::INFINITY);
                        }
                        _ => unreachable!(),
                    };
                }
            ...
            for id in &idx[*st..] {
                ...
                if let JudgeStatus::Hold(perfect, .., diff, true, _) = note.judge {
                    if let NoteKind::Hold { end_time, .. } = &note.kind {
                        if *end_time <= t {
                            note.judge = JudgeStatus::Judged;
                            judgements.push((if perfect { Judgement::ArcPerfect } else { Judgement::Good }, line_id, *id, Some(diff)));
                            continue;
                        }
                    }
                }
                ...
                if matches!(note.judge, JudgeStatus::PreJudge) {
                    ...
                    note.judge = JudgeStatus::Judged;
                    if !matches!(note.kind, NoteKind::Click) {
                        judgements.push((Judgement::ArcPerfect, line_id, *id, diff));
                    }
                }
            }
        }
        for (judgement, line_id, id, diff) in judgements {
            ...
            if match judgement {
                Judgement::ArcPerfect => {
                    res.with_model(line_tr * note.object.now(res), |res| res.emit_at_origin(note.rotation(line), res.res_pack.info.fx_arcperfect()));
                    true
                }
                Judgement::Perfect => {
    ...
    fn auto_play_update(&mut self, res: &mut Resource, chart: &mut Chart) {
        ...
        for (line_id, id) in judgements.into_iter() {
            self.commit(t, Judgement::ArcPerfect, line_id as _, id, 0.);
            ...
            res.with_model(line.now_transform(res, &chart.lines) * note_transform, |res| {
                res.emit_at_origin(line.notes[id as usize].rotation(&line), res.res_pack.info.fx_arcperfect())
            });
...
pub fn icon_index(score: u32, full_combo: bool) -> usize {
    let arcscore = ARC_SCORE_ENABLED.load(Ordering::Relaxed);
    if arcscore {

        match (score, full_combo) {
            (x, _) if x < 8_900_000 => 0,
            (x, _) if x < 9_200_000 => 1,
            (x, _) if x < 9_500_000 => 2,
            (x, _) if x < 9_800_000 => 3,
            (x, _) if x < 9_900_000 => 4,
            (x, _) if x > 10000000 => 7,
            (_, false) => 5,
            (_, true) => 6,
        }
