...
use xcsim_core_avc::AVPixelFormat;
...
impl Video {
    pub fn new(data: Vec<u8>, start_time: f32, scale_type: ScaleType, alpha: Anim<f32>, dim: Anim<f32>) -> Result<Self> {
        ...
        drop(data);
        let video = xcsim_core_avc::Video::open(video_file.path().as_os_str().to_str().unwrap(), AVPixelFormat::YUV420P)?;
    ...
    pub fn reset(&mut self) -> Result<()> {
        self.next_frame = 0;
        self.ended = false;
        self.video = xcsim_core_avc::Video::open(self.video_file.path().as_os_str().to_str().unwrap(), AVPixelFormat::YUV420P)?;
        Ok(())
    }
}
