...
xcsim_core_l10n::tl_file!("scene" ttl);
#[path = "ui_core/billboard_ui.rs"] mod billboard;
pub use billboard::{BillBoard, Message, MessageHandle, MessageKind};

#[path = "ui_core/chart_info_ui.rs"] mod chart_info;
pub use chart_info::*;

#[path = "ui_core/dialog_ui.rs"] mod dialog;
pub use dialog::Dialog;

#[path = "ui_core/scroll_ui.rs"] mod scroll;
use inputbox::{InputBox, InputMode};
pub use scroll::*;

#[path = "ui_core/shading_ui.rs"] mod shading;
pub use shading::*;

#[path = "ui_core/shadow_ui.rs"] mod shadow;
pub use shadow::*;

#[path = "ui_core/text_ui.rs"] mod text;
...
pub static DARK_THEME_UI: AtomicBool = AtomicBool::new(false);
...
impl DRectButton {
    ...
    fn hover_progress(&mut self, t: f32, hover_now: bool) -> f32 {
        if hover_now != self.hover_state {
            self.hover_state = hover_now;
            self.hover_change_time = Some(t);
        }
        if let Some(start) = self.hover_change_time {
            let elapsed = t - start;
            if elapsed >= Self::HOVER_TIME {
                self.hover_change_time = None;
            } else {
                let lin = (elapsed / Self::HOVER_TIME).clamp(0.0, 1.0);
                let eased = 1.0 - (1.0 - lin).powi(3);
                return if hover_now { eased } else { 1.0 - eased };
            }
        }
        if hover_now { 1.0 } else { 0.0 }
    }
    ...
    pub fn build(&mut self, ui: &mut Ui, t: f32, r: Rect, f: impl FnOnce(&mut Ui, Path)) {
        ...
        let press_amount = (1.0 - p).clamp(0.0, 1.0);

        let (mx, my) = mouse_position();
        let hover_now = self.inner.contains(vec2(mx, my));
        let hover_amount = self.hover_progress(t, hover_now);

        let path = r.rounded(self.config.radius);
        f(ui, path);

        let total_alpha = 0.06 * hover_amount + 0.13 * press_amount;
        if total_alpha > 0.005 {
            let dark = DARK_THEME_UI.load(Ordering::Relaxed);
            let overlay = if dark {
                Color::new(1.0, 1.0, 1.0, total_alpha)
            } else {
                Color::new(0.0, 0.0, 0.0, total_alpha)
            };
            let overlay_path = r.rounded(self.config.radius);
            ui.fill_path(&overlay_path, overlay);
        }
    }
    ...
    pub fn render_text<'a>(&mut self, ui: &mut Ui, r: Rect, t: f32, text: impl Into<Cow<'a, str>>, size: f32, chosen: bool) {
        ...
        let bg = if chosen {
            ...
            Color::new(0.22, 0.22, 0.22, 0.75)
        } else {
            Color::new(0.82, 0.82, 0.82, 0.75)
        };
        let text_c = if chosen {
            WHITE
        } else if dark {
            Color::new(0.90, 0.90, 0.90, 1.)
        } else {
    ...
    pub fn render_text_left<'a>(&mut self, ui: &mut Ui, r: Rect, t: f32, alpha: f32, text: impl Into<Cow<'a, str>>, size: f32, chosen: bool) {
        ...
        let accent = Color::new(0.949, 0.412, 0.580, 1.0);
        let bg = if chosen {
            Color::new(accent.r, accent.g, accent.b, 0.22 * alpha)
        } else if dark {
            Color::new(0.22, 0.22, 0.22, 0.55 * alpha)
        } else {
            Color::new(0.82, 0.82, 0.82, 0.55 * alpha)
        };
        let text_c = if chosen {
            Color::new(accent.r, accent.g, accent.b, alpha)
        } else if dark {
            Color::new(0.90, 0.90, 0.90, alpha)
        } else {
            Color::new(0.15, 0.15, 0.15, alpha)
        };
        self.build(ui, t, r, |ui, path| {
            ui.fill_path(&path, bg);
            if chosen {
    ...
    pub fn progress(&mut self, t: f32) -> f32 {
        ...
        let linear_p = if let Some(time) = &self.start_time {
            ((t - time) / Self::TIME).clamp(0., 1.)
        } else {
            1.
        };

        let p = if self.last_touching {
        ...
        if self.last_touching { 1. - p } else { p }
    }
...
impl Slider {
    ...
    pub fn render(&mut self, ui: &mut Ui, mut r: Rect, t: f32, p: f32, text: String) {
        ...
        let accent = Color::new(0.949, 0.412, 0.580, 1.0);
        let dark = DARK_THEME_UI.load(Ordering::Relaxed);
        let track_bg = if dark { Color::new(0.40, 0.40, 0.40, 0.9) } else { Color::new(0.70, 0.70, 0.70, 0.9) };
        ui.stroke_path(
            &{
