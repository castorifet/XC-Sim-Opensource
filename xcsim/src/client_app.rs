...
#[path = "client_app/model_client.rs"] mod model;
pub use model::*;
use tracing::debug;
...
use xcsim_core::scene::SimpleRecord;
use xcsim_core_l10n::LANG_IDENTS;
...
impl Client {
    ...
    pub async fn login(params: LoginParams<'_>) -> Result<()> {
        ...
        let resp: Resp = recv_raw(Self::post("/login", &params).timeout(Duration::from_secs(20))).await?.json().await?;
    ...
    pub async fn get_me() -> Result<User> {
        Ok(recv_raw(Self::get("/me").timeout(Duration::from_secs(20))).await?.json().await?)
    }
