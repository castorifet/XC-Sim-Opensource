...
impl UnlockScene {
    ...
    pub async fn new(
        ...
        preloaded: Option<(xcsim_core::ext::SafeTexture, xcsim_core::ext::SafeTexture, Color)>,
    ) -> Result<UnlockScene> {
        let bytes = fs
...
impl Scene for UnlockScene {
    ...
    fn render(&mut self, tm: &mut TimeManager, ui: &mut xcsim_core::ui::Ui) -> Result<()> {
        ...
        match self.state {
            ...
            State::Transforming => {
                ...
                if t < 0.5 {
                    let pad = 0.07;
                    let alpha = if t < 0.5 { 1. - t / 0.5 } else { 0. };
                    ui.loading(
                        1. - pad,
                        top - pad,
                        t,
