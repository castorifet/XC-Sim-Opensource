...
impl Page for MessagePage {
    ...
    fn render(&mut self, ui: &mut Ui, s: &mut SharedState) -> Result<()> {
        ...
        s.render_fader(ui, |ui| {
            ...
            ui.fill_rect(Rect::new(r.x, r.y, r.w, 0.003), accent);
            ui.text(tl!("label"))
            ...
            if let Some(msgs) = &mut self.msgs {
                if msgs.is_empty() {
                    ui.text(tl!("no-msg")).pos(ct.x, ct.y).anchor(0.5, 0.5).no_baseline().size(0.7).color(muted_text).draw();
                } else {
                    ui.scope(|ui| {
                        ui.dx(r.x);
                        ...
                        self.btns_scroll.render(ui, |ui| {
                            ...
                            for (index, item) in msgs.iter_mut().enumerate() {
                                ...
                                item.1.render_text_left(ui, ir, t, 1., &item.0.title, 0.5, chosen);
                                if index + 1 < count {
        ...
        s.render_fader(ui, |ui| {
            ...
            if let Some(msg) = self.index.and_then(|it| self.msgs.as_ref().map(|msgs| &msgs[it].0)) {
                ...
                dy!(ui.text(&msg.title).size(0.9).multiline().max_width(mw).color(dark_text).draw().h + 0.017);
                let th = ui.text(
                ...
                .draw().h;
                dy!(th + 0.016);
                ui.fill_rect(Rect::new(0., 0., mw, 0.003), sep_c);
                dy!(0.015);
                self.scroll.size((mw, cr.h - title_h - h - pad));
                self.scroll.render(ui, |ui| {
                    let r = ui.text(&msg.content).size(0.46).multiline().max_width(mw).color(dark_text).draw();
                    (mw, r.h + 0.04)
                });
            }
        });
