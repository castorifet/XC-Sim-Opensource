xcsim_core_l10n::tl_file!("collection");

use super::{Illustration, NextPage, Page, SharedState};
use crate::{icons::Icons, load_res_tex, resource::rtl, scene::{ChaptersScene, ChapterScene}};
use anyhow::Result;
        ...
        Ok(Self {
            ...
            colls: {
                ...
                res.push(CollectionItem {
                    id: "@bm-chapter".to_owned(),
                    illu: Illustration::from_done(Texture2D::from_rgba8(1, 1, &[64, 122, 207, 255]).into()),
                    title: "BM Chapter".to_string(),
                    btn: RectButton::new(),
                });
                res.push(CollectionItem {
                    id: "@".to_owned(),
                ...
                if coll.btn.touch(touch) {
                    if coll.id != "@" {
                        ...
                        self.scene_task = Some(Box::pin(async move {
                            let scene: NextScene = if id == "@bm-chapter" {
                                let _ = illu;
                                NextScene::Overlay(Box::new(ChaptersScene::new(icons, rank_icons)))
                            } else {
                                NextScene::Overlay(Box::new(ChapterScene::new(id, icons, rank_icons, illu).await?))
                            };
                            Ok(scene)
                        }));
                    }
