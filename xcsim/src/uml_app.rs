#[path = "uml_app/lexer_uml.rs"] mod lexer;
#[path = "uml_app/parse_uml.rs"] mod parse;
...
impl Element for Text {
    ...
    fn render(&self, ui: &mut Ui, uml: &Uml) -> Result<Var> {
        ...
        Ok(Var::Rect(match c.id.as_deref() {
            Some("pgr") => xcsim_core::core::PGR_FONT.with(|it| text.draw_with_font(it.borrow_mut().as_mut())),
            Some("bold") => text.draw(),
            _ => text.draw(),
        }))
    }
