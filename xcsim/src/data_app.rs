...
impl Data {
    pub async fn init(&mut self) -> Result<()> {
        ...
        #[cfg(all(target_os = "ios", target_os = "android"))]
        for entry in std::fs::read_dir(moddir::asset_charts()?)? {
    let entry = entry?;
    let filename = entry.file_name();
    let filename = filename.to_str().unwrap();
    let local_path = format!("special/{filename}");
    if occurred.contains(&local_path) {
        continue;
    }
    let path = entry.path();
    let Ok(mut fs) = xcsim_core::fs::fs_from_file(&path) else {
        continue;
    };
    let result = xcsim_core::fs::load_info(fs.deref_mut()).await;
